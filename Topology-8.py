...
def convert_txt_to_edges(txt_file: str, sn_map: dict) -> List[Tuple[str, str, str]]:
    ...
    - CFW/FSW 之間的連線，去重只保留第一條。
    - FSW/FSW 之間的連線，保留所有連線。
    """
    edges_list = []
    cfw_fsw_processed_keys = set()
    ...
    for line in lines:
        ...
        # 檢查是否為 FSW 對 FSW 的連線
        is_fsw_to_fsw = "FSW" in left_dev and "FSW" in right_dev

        if is_fsw_to_fsw:
            # FSW-FSW 連線，直接加入列表，不進行去重
            edges_list.append((left_dev, right_dev, label))
        else:
            # 其他連線 (CFW-FSW, CFW-CFW)，進行去重
            key = tuple(sorted([left_dev, right_dev]))
            if key not in cfw_fsw_processed_keys:
                cfw_fsw_processed_keys.add(key)
                edges_list.append((left_dev, right_dev, label))
...
def extract_nodes_from_edges(edges_list: List[Tuple[str, str, str]]):
    ...
    return sorted(list(cfw_nodes)), sorted(list(fsw_nodes))
...
def main():
    ...
    # 5) 將 edges_list 轉換成最終的 Graphviz 格式字串
    edge_lines = []
